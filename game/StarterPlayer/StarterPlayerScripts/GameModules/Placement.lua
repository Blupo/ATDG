...
local doesCollide = function(part1CFrame: CFrame, part1Size: Vector3, part2CFrame: CFrame, part2Size: Vector3): boolean
	...
	for i = 1, #axis do
		...
		if ((scalars1Min > scalars2Max) or (scalars2Min > scalars1Max)) then return false end
	end
...
Placement.CanPlace = function(objType: string, objName: string, position: Vector3, rotation: number): PlacementResult	
	local thisObjModel
	...
	-- todo: check that the user has enough points to place it down
